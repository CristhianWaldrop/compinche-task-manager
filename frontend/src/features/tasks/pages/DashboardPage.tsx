...
import { CreateTaskModal } from '../components/CreateTaskModal'; // <-- Importamos el modal
...
export const DashboardPage: React.FC = () => {
  ...
  const queryClient = useQueryClient(); // Acceso al cliente global de caché
  ...
  // Mutación para crear la tarea vinculada a la caché
  const createMutation = useMutation({
    mutationFn: tasksService.createTask,
    onSuccess: () => {
      setApiError(null);
      setIsModalOpen(false); // Cerramos el modal
      queryClient.invalidateQueries({ queryKey: ['tasks'] }); // Refrescamos la lista automáticamente
    },
    onError: (error: unknown) => {
      if (axios.isAxiosError(error) && error.response) {
        const dataError = error.response.data as ApiError;
        setApiError(Array.isArray(dataError.message) ? dataError.message.join(', ') : dataError.message);
      } else {
        setApiError('Error de red al intentar crear la tarea.');
      }
    },
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Cambiamos el comportamiento del botón pasándole una función para abrir el modal */}
        <div className="mb-6" onClick={() => setIsModalOpen(true)}>
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-500 text-sm">Cargando tus tareas...</p>
          </div>
        )}

        {isError && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-2xl text-center text-red-700 text-sm">
            Ocurrió un error al intentar sincronizar las tareas.
          </div>
        )}
          ...
          <>
            {data?.data.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-2xl border border-slate-100 p-8">
                <p className="text-slate-400 font-medium">No se encontraron tareas. ¡Crea una nueva para empezar!</p>
              </div>
            ) : (
            ...
            {data && data.meta.totalPages > 1 && (
              <div className="mt-8 flex items-center justify-between border-t border-slate-200/60 pt-4">
                <span className="text-xs text-slate-500 font-medium">
                  Mostrando página {data.meta.page} de {data.meta.totalPages} ({data.meta.total} tareas en total)
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={() => handlePageChange(filters.page - 1)}
                    disabled={filters.page === 1}
                    className="px-3 py-1.5 text-xs font-semibold bg-white border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
                  >
                    Anterior
                  </button>
                  <button
